  ...
  processEntry (fields, options) {
    ...
    return this.getSiteConfig().then(config => {
      ...
      if (this.siteConfig.get('moderation')) {
        const now = new Date();
        const timestamp = `${now.getFullYear()}-${(now.getMonth() + 1).toString().padStart(2, '0')}-${now.getDate().toString().padStart(2, '0')}__${now.getHours().toString().padStart(2, '0')}-${now.getMinutes().toString().padStart(2, '0')}-${now.getSeconds().toString().padStart(2, '0')}`;
        const newBranch = `submission_${timestamp}`;
